...
class Model:
    ...
    def plot(self, **kwargs) -> figure.Figure:
        ...
        (mu_minus_3sigma - df["sigma"]).plot(
            ax=ax, color="purple", label="$\\pm4\\sigma$", alpha=0.5
        )
        ...
        anomalies = df[
            (mu_minus_3sigma >= df["delta_rtt"]) | (df["delta_rtt"] >= mu_plus_3sigma)
        ]
        anomalies.plot(
            y="delta_rtt",
            ax=ax,
        ...
        if prob_changes:
            # plot probability
            ax: plt.Axes = axs[1]
            df["success_prob"].plot(
                ax=ax, label="$P(\\mathrm{success})$", color="green"
            )
        ...
        if ttl_changes:
            ...
            ax: plt.Axes = axs[-1]
            df["delta_ttl"].plot(
                kind="line", ax=ax, label="$\\Delta(\\mathrm{ttl})$", color="red"
            )
    ...
    def score(
        self, rtt, ttl, destination_reached
    ) -> tuple[bool, float, float, float, float, float]:
        rtt_is_outlier = (
            rtt > self.mu + 3 * self.sigma or rtt < self.mu - 3 * self.sigma
        )
