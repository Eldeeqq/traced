...
class TraceMonitor:
    ...
    def log(self, data):
        ...
        asn_hash = sha1(''.join(list(map(str, data["asns"]))).encode('utf-8')).hexdigest()
        self.detected[self.n]['ip_hash'] = sha1(''.join(list(map(str, data["hops"]))).encode('utf-8')).hexdigest()
        ...
        for x in data['asns']:
            if tmp and tmp[-1] == x:
                continue
            tmp.append(x)
        self.detected[self.n]['unique_asns'] = sha1(''.join(list(map(str, tmp))).encode('utf-8')).hexdigest()
        ...
        asn_hash = sha1(''.join(["src"]+list(map(str, data["asns"]))).encode('utf-8')).hexdigest()
        tmp = self.detected[self.n]['path'] = self.path_probs.log(ts, asn_hash)
        is_anomaly = is_anomaly or tmp[0]
        anom_cnt += int(tmp[0])
        total_cnt += 1

        tmp = self.detected[self.n]['reached'] = self.destination_reached_model.log(ts, data["destination_reached"])
        is_anomaly = is_anomaly or tmp[0]
        anom_cnt += int(tmp[0])
        total_cnt += 1
